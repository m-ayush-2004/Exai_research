...
def dynamic_n_segments_for_coverage(image, target_coverage=0.10, quickshift_kwargs=None):
    if quickshift_kwargs is None:
        quickshift_kwargs = dict(kernel_size=4, max_dist=200, ratio=0.2)
    superpixels = quickshift(image, **quickshift_kwargs)
    unique_segments, counts = np.unique(superpixels, return_counts=True)
    total_pixels = superpixels.size
    print(total_pixels)
    coverage_percent = counts / total_pixels
    print(coverage_percent)

    # Sort segments by percent descending
    sorted_indices = np.argsort(-coverage_percent)
    sorted_segments = unique_segments[sorted_indices]
    sorted_coverage = coverage_percent
    print(sorted_coverage)

    # Cumulative selection for >=10%
    cumulative = 0.0
    n = 0
    for c in sorted_coverage:
        ...
        if cumulative >= target_coverage:
            ...
            break
        print(cumulative)
    return max(n, 1)  # SLIC requires at least 2 segments
