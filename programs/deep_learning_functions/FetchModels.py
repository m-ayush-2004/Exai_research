...
from deep_learning_functions.LoadConfig import load_config
import os 
import json
...
def train_and_save_model(model, model_name, disease_name, epochs=30, fetch_existing=True, train_generator= None, val_generator = None):
    ...
    """
    # Create save paths
    base_dir = f"Datasets/weights/{model_name}/{disease_name}"
...
def load_model_from_weights(model_name, disease_name, config_path=DL_CONFIG_FILE_PATH):
    ...
    if model_name in models and disease_name in models[model_name]:
        ...
        labels  = models[model_name][disease_name]["class_name"]
        return model , labels
    else:
