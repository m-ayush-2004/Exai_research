...
def predict(file , disease_name , model_name):
    ...
    model,labels = load_model_from_weights(model_name,disease_name)
    ...
    segment_info,path,shap_path, lime_path = generate_lime_heatmap_and_explanation(model=model, image=image_array[0],num_segments_to_select=2, model_name=model_name, disease_name=disease_name , labels=labels)
